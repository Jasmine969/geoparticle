...
line.plot(ax01)
symm_lines = SymmLines(2, 'z', 0.3, dl, 'symm_lines').shift(y=-2)
...
circle = Circle(1.5, 0, 'XOY', dl, 'circle').shift(x=2)
circle.plot(ax02)

torus2D = Torus2D(0.4, 1.2, dl, plane='XOY', phi_range='[0,270)').shift(x=-1, y=4)
torus2D.plot(ax02)
...
# rectangles
rectangle = Rectangle(4, 1.2, 0, 'z', dl).shift(y=3)
rectangle.plot(ax11)
filled_rectangle = FilledRectangle(1.7, 2, 0, 'z', dl)
filled_rectangle.plot(ax11)
thick_rectangle = ThickRectangle(1.7, 2, 2, 0, 'z', dl).shift(x=2.5)
thick_rectangle.plot(ax11)
ax11.set_title('rectangles')
ax11.axis('equal')
# circles
ax12 = fig1.add_subplot(122)
circle = Circle(1.5, 0, 'XOY', dl, 'circle').shift(x=3,y=-1)
circle.plot(ax12)
thick_ring = ThickRing(1.6, 1, dl, incl_inner=True, incl_outer=True, axis='z').shift(y=1)
...
block = Block(3, 4, 5, dl)
thick_block_wall = ThickBlockWall(3, 4, 5, 2, dl).shift(x=5)
clipped_wall = Clip(thick_block_wall, keep='negative', plane_point=(6.5, 2, 2.5), plane_normal=(0, 0, 1))
...
tube = CylinderSide(2, 10, dl, 'z')
thick_tube = ThickCylinderSide(2, 1.5, 10, dl, 'z').shift(x=5)
water_column = FilledCylinder(1.8, 10, dl, 'z').shift(x=10)
tube.plot(ax22)
...
torus_surface = TorusSurface(2, 5, dl, plane='XOY', phi_range='[0,150)')
thick_torus = ThickTorusWall(2, 5, 2, dl, plane='XOY', phi_range='[0,150)').shift(z=8)
filled_torus = FilledTorus(2, 5, dl, plane='XOY', phi_range='[0,150)').shift(z=16)
...
sphere_surface = Clip(SphereSurface(3, dl, 'sphere_surface'), keep='negative',
                      plane_point=(0,0,0), plane_normal=(0,0,1))
sphere_shell = Clip(ThickSphere(3, 2, dl), keep='negative',
                    plane_point=(0,0,0), plane_normal=(0,0,1))
sphere = Clip(FilledSphere(3, dl, 'sphere'), keep='negative',
              plane_point=(0, 0, 0), plane_normal=(0, 0, 1))
def calc_rs(coords):
...
torus_surface = TorusSurface(2, 5, dl, n_per_ring(2, dl), regular_id=False,
                     plane='XOZ', phi_range='[0,360)').shift(y=-5)
block =  Block(3, 7, 15, dl).shift(x=-5,y=-8,z=-7)
intersect = Intersect((torus_surface, block), rmax=dl).shift(x=-5,z=-15)
subtract = Subtract(torus_surface, block, rmax=dl).shift(x=5, z=-15)
union = Union((torus_surface, block)).shift(x=23, z=-15)
...
clip = Clip(thick_block_wall, keep='negative', plane_point=(1, 0, 3), plane_normal=(0, 0, 1)).shift(x=8)
